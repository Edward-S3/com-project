...
def template_requires_audio(tmpl: dict | None) -> bool:
    ...
    if not tmpl:
        return False
    return (tmpl.get("category") or "").strip() == "音声"
...
def template_is_image_generation(tmpl: dict | None) -> bool:
    ...
    if not tmpl:
        return False
    return (tmpl.get("category") or "").strip() == "画像"
...
def _parse_gemma4_install(name: str) -> tuple[str, dict[str, str]] | None:
    ...
    if not meta:
        for key, m in GEMMA4_TAGS.items():
            if tag.startswith(key) or key in tag:
                meta = m
                tag = key
                break
    if not meta:
...
def get_available_models() -> dict[str, str]:
    """API キー設定済み + Ollama インストール済みのモデルのみ"""
    reload_provider_env()
    result: dict[str, str] = {}
...
def _attachment_model_priority() -> list[str]:
    return [
        ...
        "local:gemma4-26b", "local:gemma4-31b", "local:gemma4-e4b",
    ]
...
def _heuristic_auto_select(
    ...
    if locals_only:
        ...
        if hard:
            for m in ("local:gemma4-26b", "local:gemma4-31b", "local:gemma4-e4b"):
                if m in candidates:
                    return m
        for m in ("local:gemma4-e4b", "local:gemma4-26b"):
            if m in candidates:
                return m
    ...
    priority = [
        ...
        "local:gemma4-e4b", "local:gemma4-26b", "local:gemma4-31b",
    ...
    if any(h in low or h in prompt for h in local_hints):
        for m in ("local:gemma4-26b", "local:gemma4-e4b", "local:gemma4-31b"):
            if m in candidates:
                return m

    if any(h in low or h in prompt for h in hard_hints):
        for m in ("claude-opus-4-6", "gemini-3.1-pro-preview", "gpt-5.5", "gpt-5.4", "local:gemma4-26b"):
            if m in candidates:
                return m
    if any(h in low or h in prompt for h in code_hints):
        for m in ("gpt-5.5", "claude-sonnet-4-6", "gemini-3.1-pro-preview", "gpt-5.4", "local:gemma4-26b"):
            if m in candidates:
                return m
    for m in priority:
        if m in candidates:
...
def auto_select_model(
    ...
    if ollama_router_available():
        ...
        system = (
            ...
            "機密・社内データ・オフライン希望には local:gemma4-e4b または local:gemma4-26b を優先してください。"
            "添付ファイル（画像/PDF）がある質問はマルチモーダル対応のクラウドまたは local:gemma4-26b を選んでください。"
        )
...
_OFFICE_OUTPUT_RE = re.compile(r"\n?<!--NAI_OFFICE:[A-Za-z0-9+/=]+-->\s*")
...
def stream_response(
    ...
    if atts and use_web_search:
        raise ValueError("ファイル添付と Web 検索は同時に利用できません。")
