...
def page_templates() -> None:
    ...
    tab_list, tab_add = st.tabs(["テンプレート一覧・編集", "新規テンプレート追加"])
    ...
    with tab_list:
        ...
        df = df[[
            "id", "name", "category", "default_model_label",
            "empty_prompt_label", "is_active", "sort_order", "created_at",
        ]]
        df.columns = [
            "ID", "名称", "カテゴリ", "デフォルト LLM",
            "空プロンプト", "有効", "順序", "作成日時",
        ]
        ...
        sel_id = st.selectbox(
            "編集するテンプレート",
            valid_ids,
            format_func=lambda i: f"[{i}] {next(t['name'] for t in templates if t['id'] == i)}",
            key="tmpl_edit_sel",
        )
            ...
            if st.form_submit_button("保存", width="stretch"):
                ...
                if new_id != int(t["id"]) and db.template_id_exists(new_id):
                    ...
                    err = db.update_template(
                        ...
                        1 if et_active else 0, et_order,
...
def page_settings() -> None:
    ...
    st.info("ここで設定した値はすべてのユーザーのチャットに即時反映されます。")
