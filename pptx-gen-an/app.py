...
from src.anthropic_client import AnthropicClientError, generate_presentation_data
...
def main() -> None:
    ...
    st.markdown(
        "ソース資料と利用目的から、Claude が**章立て目次 → 章ごと分割生成**でスライド JSON を設計し、"
    ...
    try:
        ...
        with st.status("Claude で分割生成中...", expanded=True) as status:
