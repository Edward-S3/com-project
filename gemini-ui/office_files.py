...
_EXCEL_EXTRA_MARKER_RE = re.compile(
    r"\n?<!--NAI_EXCELEXTRA:([A-Za-z0-9+/=]+)-->\s*$",
)
...
def clean_dataframe_data_only(df: pd.DataFrame) -> pd.DataFrame:
    """空行・空列を除去しデータ領域のみ残す"""
    if df.empty:
        return df
    work = df.replace("", pd.NA).dropna(how="all", axis=0).dropna(how="all", axis=1)
    return work.fillna("")


def load_xlsx_data_only(path: str) -> dict[str, pd.DataFrame]:
    """xlsx ファイルからデータのみ（空行・空列除去済み）を読み込む"""
    with open(path, "rb") as fp:
        sheets = load_xlsx_sheets(fp.read())
    return {name: clean_dataframe_data_only(df) for name, df in sheets.items()}


def sheets_data_only_text(
    sheets: dict[str, pd.DataFrame],
    *,
    max_rows: int = 100,
) -> str:
    """データのみ抽出済みシートを LLM 送信用テキストに変換"""
    if not sheets:
    ...
    for name, df in sheets.items():
        ...
        parts.append(
            ...
            f"{view.to_csv(index=False)}"
        )
        if len(df) > max_rows:
            parts.append(f"（先頭 {max_rows} 行のみ）")
    return "\n\n".join(parts)


def list_aggregatable_sheets(path: str) -> list[dict]:
    """集計可能なシート（データあり）の概要一覧"""
    try:
        sheets = load_xlsx_data_only(path)
    except Exception:
        return []
    result: list[dict] = []
    for name, df in sheets.items():
        if df.empty or len(df.columns) == 0:
            continue
        num_cols = len(numeric_columns(df))
        result.append({
            "name": str(name),
            "rows": len(df),
            "cols": len(df.columns),
...
def filter_sheets_by_names(
    sheets: dict[str, pd.DataFrame],
    names: list[str] | None,
) -> dict[str, pd.DataFrame]:
    """選択シート名で DataFrame 辞書を絞り込む"""
    if not names:
        return {}
    allowed = set(names)
    return {k: v for k, v in sheets.items() if k in allowed}
...
def workbook_bytes_from_rows(
    rows: list[list[str]],
    *,
    sheet_name: str = "Sheet1",
) -> bytes:
    """可変列の行データを xlsx 1シートへ（表は列ごとにセル分割）"""
    import openpyxl
...
def _decode_output_payload(encoded: str) -> list[dict]:
    payload = json.loads(base64.standard_b64decode(encoded).decode())
    return [
        {
            "name": item["name"],
            "mime": item["mime"],
...
def serialize_excel_extra_outputs(outputs: list[dict]) -> str:
    if not outputs:
        return ""
    payload = [
        {
    ...
    return f"\n<!--NAI_EXCELEXTRA:{encoded}-->"


def split_excel_extra_outputs(content: str) -> tuple[str, list[dict]]:
    m = _EXCEL_EXTRA_MARKER_RE.search(content or "")
    if not m:
        return content or "", []
    text = (content or "")[: m.start()].rstrip()
    try:
        return text, _decode_output_payload(m.group(1))
    except Exception:
        return content or "", []


def parse_stored_message(content: str) -> tuple[str, list[dict], list[dict]]:
    """DB 保存形式から表示テキスト・Office 出力・ExcelExtra 出力を復元"""
    text = content or ""
    office_outputs: list[dict] = []
    excel_extra_outputs: list[dict] = []
    m_off = _OFFICE_MARKER_RE.search(text)
    if m_off:
        try:
            office_outputs = _decode_output_payload(m_off.group(1))
        except Exception:
            pass
        text = text[: m_off.start()].rstrip()
    m_ex = _EXCEL_EXTRA_MARKER_RE.search(text)
    if m_ex:
        try:
            excel_extra_outputs = _decode_output_payload(m_ex.group(1))
        except Exception:
            pass
        text = text[: m_ex.start()].rstrip()
    return text, office_outputs, excel_extra_outputs


def split_content_and_outputs(content: str) -> tuple[str, list[dict]]:
    text, office_outputs, _ = parse_stored_message(content)
    return text, office_outputs
