...
作成日: 2026-06-08  
最終更新: 2026-06-12（添付+Web検索・Excel集計・メンテナンスモード・調査テンプレート等）
...
├── excel_runner.py         # Excel集計・分析テンプレート（pandas + Gemini 2段階）
├── template_registry.py    # テンプレート種別（template_kind）メタデータ
├── maintenance_log.py      # メンテナンスモード時の実行プロセス追跡
...
| テンプレート一覧 | チャットサイドバー | **`db.get_active_templates_for_user()`** — 下記「テンプレート表示フィルタ」参照 |
| 初期テンプレート | ログイン時 / 新規チャット | **ID=1「汎用アシスタント」**（許可モデルに合う場合） |
| 実行時モデル | テンプレート使用中 | `resolve_effective_model()` — テンプレート LLM → ユーザー既定 → サイドバー選択。Web検索 ON 時は Gemini 優先 |

#### テンプレート表示フィルタ（利用者サイドバー）

**一覧に含まれる条件（すべて）:**

- `prompt_templates.is_active = 1`
- テンプレート `default_model` が **空** → 常に表示
- ユーザー `allowed_models` が **空（全モデル可）** → 有効テンプレートはすべて表示
- 上記以外 → テンプレート `default_model` がユーザーの許可モデルに **含まれる** もののみ

**一覧では判定されないもの（表示はされるが実行/UI で制限）:**

- Web検索許可、添付上限・形式、テンプレート種別（音声/Excel集計/画像）の実行要件
- `.env` の API キー未設定で実際には使えないモデル（`runtime_models` はキー設定反映後）

管理画面のテンプレート一覧は **全テンプレート** を表示（権限フィルタなし）。
...
| query_logs | 利用ログ |
| prompt_templates | プロンプトテンプレート（**default_model**, **allow_empty_prompt**, **template_kind**, **handler_config**, **sort_order**） |
| feedback / daily_usage / settings | 評価・日次制限・システム設定 |
| maintenance_debug_logs | **メンテナンスモード**時の実行プロセス追跡ログ（通常運用へ戻すと全削除） |
...
- Excel 集計テンプレート: `ensure_excel_extra_template()`（`template_kind=excel_extra`）
- 添付+Web 調査テンプレート: `ensure_web_research_templates()`（5 件、Gemini 既定）
- メンテナンスモード: `settings.maintenance_mode`（0/1）、`maintenance_debug_logs` テーブル

### テンプレート種別（`template_kind` — `template_registry.py`）

| kind | 用途 | モデル制約 |
|------|------|-----------|
| `standard` | 通常チャット | なし |
| `excel_extra` | Excel 集計・分析（2 段階: pandas → Gemini） | **Gemini のみ** |
| `audio` | 音声文字起こし・議事録 | **Gemini のみ** |
| `image` | 画像生成 | **gemini-2.5-flash-image のみ** |
| `office_output` | docx/pptx 自動出力 | なし |

### プロンプトテンプレート（2026-06-12 時点・抜粋）

| ID | 名称 | kind | デフォルト LLM | sort_order | 備考 |
|----|------|------|----------------|------------|------|
| 1 | 汎用アシスタント | standard | （未指定） | 1 | 欠落時のみ `ensure_core_templates()` で復元 |
| 14 | Excel集計・分析 | excel_extra | gemini-3.1-pro-preview | 1 | xlsx 必須・シート選択必須 |
| 15–19 | 資料×Web比較調査 等 | standard | gemini-3.5-flash | 12–16 | 添付+Web検索向け（`ensure_web_research_templates()`） |
| 10 | イメージデータ生成 | image | gemini-2.5-flash-image | 11 | |
| 11–12 | 音声文字起こし / 議事録 | audio | gemini-3.5-flash | 20 | **空プロンプト可** |

- 管理画面・利用者画面の一覧順: **`sort_order`, `id` 昇順**（`get_all_templates()` / `get_active_templates()`）
- テンプレート編集保存後は widget キーをクリアして再表示（`admin.py` `_clear_tmpl_edit_widget_keys`）
...
- Web 検索（Gemini のみ。**添付ありでも Gemini なら同時利用可** — 下記参照）
- **ファイル添付**（画像 / PDF / テキスト / **Office（xlsx, docx, pptx）** / **音声**）
- **Excel 集計・分析**（`template_kind=excel_extra`）— xlsx 必須・シート選択 → pandas 集計 → Gemini 分析。**集計結果は `st.dataframe` + CSV ダウンロード**
- **添付 + Web 検索** — 資料×Web 比較調査等（テンプレート ID 15–19）。Gemini + Web 検索 ON 時のみ
...
- **画像生成**（`gemini-2.5-flash-image`、テンプレート ID=10）
...
- **メンテナンスモード時** — `maintenance_log.py` 経由で `gen_task` 実行プロセスを DB 追跡（利用者画面にバナーは出さない）

### 添付 + Web 検索（2026-06-12）

| 条件 | 動作 |
|------|------|
| Gemini モデル + Web 検索 ON + 添付あり | **許可**（Google Search Grounding と添付を同時送信） |
| 非 Gemini / Excel集計 / 音声 / 画像生成テンプレート | 従来どおり **同時利用不可** または Web 検索無効 |
| モデル解決 | `resolve_effective_model(..., need_web_search=True)` — Web 検索 ON 時は許可 Gemini を優先 |

実装: `llm_providers.py` の `stream_response`（旧: 添付ありで Web 検索を一律ブロックしていた）

### Excel 集計・分析（`excel_runner.py`）

1. ユーザーが xlsx 添付 + シート選択 + プロンプト送信
2. **前処理** — Gemini が pandas コード生成 → 実行 → `### 集計結果` セクションに CSV 形式出力
3. **分析** — 集計結果をコンテキストに Gemini が傾向・考察をストリーミング
4. UI — `_render_excel_extra_aggregated_data()` で集計を `st.dataframe` 表示、**集計結果（CSV）** ダウンロード

集計が有効な出力かは `_is_usable_preprocess_output()`（複数行 CSV 形式）で判定。
...
| テンプレート管理 | CRUD・**表示順（sort_order）**・ID 編集・デフォルト LLM・**空プロンプト可**・再表示・削除確認 |
| システム設定 | グローバル設定・**添付ファイル上限/形式**・**メンテナンスモード ON/OFF**・LLM 更新ボタン |
| **🔧 メンテナンスログ** | メンテナンス ON 中の実行トレース一覧・詳細（フェーズ・ユーザー権限スナップショット） |

**メンテナンスモード（2026-06-12）:**

- 管理画面 → システム設定 → **メンテナンスモード** トグル（`settings.maintenance_mode`）
- ON 中: 利用者の各 `gen_task` を trace 単位で `maintenance_debug_logs` に記録（routing / excel フェーズ / complete / error）
- trace 開始時に `db.get_user_execution_snapshot()` — 許可 LLM・Web 検索・添付上限・日次制限等（**パスワード/API キーは含めない**）
- **通常運用に戻す** と全ログを `purge_maintenance_logs()` で削除
- 利用者画面にはメンテナンスバナーは未実装（管理者のみログ閲覧）
...
### Web 検索（Google Search Grounding）

- Gemini モデルのみ。`stream_response` 内で Grounding ツールを付与
- **添付ファイルと同時利用可**（Gemini のみ。2026-06-12 以降）
- Excel 集計・音声・画像生成テンプレートでは UI/API 双方で無効
...
### 2026-06-12 セッション（テンプレート順序・Web検索・Excel・メンテナンス）

| 項目 | 内容 |
|------|------|
| **テンプレート表示順** | `get_all_templates()` を `ORDER BY sort_order, id` に修正。保存後 widget キークリア |
| **添付 + Web 検索** | Gemini 限定で同時利用を許可。`resolve_effective_model(..., need_web_search=True)` |
| **Web 調査テンプレート 5 件** | ID 15–19（`ensure_web_research_templates()`）。資料比較・競合調査・規程チェック等 |
| **Excel 集計結果表示** | `excel_runner.py` 集計セクション厳格化 + `app.py` で dataframe/CSV 表示 |
| **メンテナンスモード** | `maintenance_log.py` + `maintenance_debug_logs` + 管理画面トグル/ログ閲覧 |
| **実行時権限スナップショット** | `get_user_execution_snapshot()` をメンテナンス trace に記録 |
...
| テンプレート表示順が変わらない | `get_all_templates()` が `ORDER BY id` のみ | `sort_order, id` に修正済み |
| Excel 集計で集計表が出ない | 前処理出力判定が緩く分析のみ表示 | `_is_usable_preprocess_output` + UI 分離 |
| 添付 + Web 検索が使えない | `stream_response` で一律ブロック | Gemini のみ許可に変更 |
...
| `/opt/gemini-ui/excel_runner.py` | Excel 集計・分析（pandas + Gemini 2 段階） |
| `/opt/gemini-ui/template_registry.py` | テンプレート種別メタデータ |
| `/opt/gemini-ui/maintenance_log.py` | メンテナンスモード実行追跡 |
| `/opt/gemini-ui/app.py` | 利用者 UI（添付・Office・Excel集計・メンテナンス trace） |
...
```bash
appctl status nai naictrl
cd /opt/gemini-ui && venv/bin/python3 -m py_compile app.py admin.py db.py llm_providers.py office_files.py excel_runner.py maintenance_log.py template_registry.py sync_env_job.py ui_common.py
venv/bin/python3 -c "
import db; db.init_db()
print('maintenance:', db.maintenance_mode_enabled())
...
for t in db.get_all_templates():
    if t['id'] >= 10 or t.get('template_kind') != 'standard':
        print(' ', t['id'], t['name'], t.get('template_kind'), 'sort=', t.get('sort_order'))
"
ollama list
...
```
app.py / admin.py
  ├─ llm_providers.py     … LLM / 音声 / 画像 / Office 添付 / Web 検索 Grounding
  ├─ office_files.py      … xlsx 編集 / docx・pptx 出力
  ├─ excel_runner.py      … Excel 集計・分析（2 段階パイプライン）
  ├─ template_registry.py … template_kind メタデータ
  ├─ maintenance_log.py   … メンテナンスモード実行 trace
  ├─ ui_common.py         … バナー / ログイン CSS
  ├─ db.py                … SQLite（テンプレート・許可モデル・メンテナンスログ）
...
1. テンプレート「資料×Web比較調査」等（ID 15–19）を選択
2. PDF や txt 等を添付し、Web 検索を ON
3. 比較・調査系のプロンプトを送信
4. 添付内容と Web 情報の両方を参照した回答が返ること（Gemini モデルであること）
5. 非 Gemini モデルまたは Excel集計/音声/画像テンプレートでは同時利用不可であること

### Excel 集計・分析の動作確認手順（参考）

1. テンプレート「Excel集計・分析」を選択
2. xlsx を添付 → シートを選択 → 集計指示を送信
3. **集計結果** がテーブル（`st.dataframe`）で表示されること
4. **集計結果（CSV）** ダウンロードが利用できること
5. その下に分析・傾向のテキストが表示されること

### メンテナンスモードの動作確認手順（参考）

1. 管理画面 → システム設定 → メンテナンスモード ON
2. 利用者画面でチャット実行（テンプレート変更・添付等も試す）
3. 管理画面 → **🔧 メンテナンスログ** — trace 一覧・詳細にフェーズと `user_permissions` が記録されていること
4. メンテナンスモード OFF → ログが全削除されること
